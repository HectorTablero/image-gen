        ...
        **kwargs
        ...
        try:
            ...
            noise_scale = torch.sqrt(torch.tensor(
                2.0 * self.corrector_snr, device=x_t.device))
            noise = torch.randn_like(x_t)

            # Calculamos la norma del score cuidadosamente
            # Usamos un valor pequeño de epsilon para evitar división por cero
            epsilon = 1e-10
            score_norm = torch.norm(score.view(
                score.shape[0], -1), dim=1, keepdim=True).view(-1, 1, 1, 1)
            score_norm = torch.maximum(score_norm, torch.tensor(
                epsilon, device=score_norm.device))

            # Calculamos el tamaño del paso con broadcasting correcto
            step_size = (self.corrector_snr /
                         (score_norm ** 2)).view(-1, 1, 1, 1)
            ...
            if self.verbose:
                print(
                    f"IndexError en corrector_step: {e}. Omitiendo corrección.")
        ...
        **kwargs
        ...
        for i in iterable:
            ...
            try:
                ...
                if self.verbose:
                    print(
                        f"Error en el paso corrector: {e}. Continuando sin corrección.")
            ...
            if guidance is not None:
                try:
                    ...
                    if self.verbose:
                        print(
                            f"Error en guidance: {e}. Continuando sin aplicar guía.")
